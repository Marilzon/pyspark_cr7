...
def scd1_merge(
    ...
    date_compare_conditions = " AND ".join(
        [f"source.{date} > target.{date}" for date in date_deduplicate_cols]
    )
...
animals_df.write.format("delta").mode("overwrite").partitionBy(
    "__catalog_insert_date"
).saveAsTable("animals")
...
new_animals_df = spark.read.option("header", True).csv(
    "pyspark_cr7/scd1/data/updated_animals.csv"
)
...
scd1_merge(
    ...
    new_dataframe=new_animals_df,
    ids=["id"],
    date_deduplicate_cols=["created_date"],
)
